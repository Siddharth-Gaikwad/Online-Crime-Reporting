class BranchesController < ApplicationController
  ...
  # 🔹 NEW ACTION: Find nearest branch
  def nearest
    lat = params[:lat].to_f
    lon = params[:lon].to_f
    branch = Branch.nearest_branch(lat, lon)

    if branch
      render json: { id: branch.id, name: branch.name }
    else
      render json: {}
    end
  end
  ...
  private
    ...
    def branch_params
      params.require(:branch).permit(:name, :address, :city, :state, :pincode, :latitude, :longitude)
    end
end
